...
//         $chunkSize = 500; // Reduced from 5000 for better memory management
//         $total = 5000; // Reduced from 500000 for testing purposes
...
class ProductSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $chunkSize = 500;
        $total     = 2000; // scale later

        for ($i = 0; $i < $total; $i += $chunkSize) {

            DB::transaction(function () use ($chunkSize, $colors, $sizes, $tags) {

                // 1️⃣ Create products (NO events, NO ES)
                $products = Product::factory()
                    ->count($chunkSize)
                    ->make();
                ...
                // Get inserted IDs
                $insertedProducts = Product::latest('id')
                    ->take($chunkSize)
                    ->get();
                ...
                foreach ($insertedProducts as $product) {
                    ...
                    // 4️⃣ Tags
                    $selected = (array) array_rand($tags, rand(2, 3));
